...
const TodoList: React.FC<TodoListProps> = ({categoryName}) => {
    ...
    return (
        <ul>
            {state[categoryName].todos.map(todo => <li draggable onDragLeave={onDrag} onClick={() => updateStatus(categoryName, todo.id, todo.isCompleted ? 'Pending' : 'Completed')} className={todo.isCompleted ? 'completed relative cursor-pointer' : 'pending relative cursor-pointer'} key={todo.id}>
